...
> **Platform repo:** [vinayak-rz/stamped-external](https://github.com/vinayak-rz/stamped-external) · **ADR:** [ADR-011](../../../decisions/011-015/ADR-011-stamped-platform-submodule-distribution.md)
...
| `external/scripts/contracts/contract-check.sh` | Shared CI validation | **No** |
...
3. **Dedupe golden** — `external/contracts/fixtures/golden/dedupe_golden.json` is authoritative. All L1/L2 dedupe tests must match it exactly.
...
- CI must run `./external/scripts/contracts/contract-check.sh` on every PR.
- Pin `external/` to a release tag (e.g. v2026.07.12), not main.

Dedupe authority: `external/contracts/fixtures/golden/dedupe_golden.json`.
...
```markdown
Platform SSOT: `external/` submodule → [stamped-external](https://github.com/vinayak-rz/stamped-external) @ tag pinned in this PR. Contract/ADR changes require stamped-external PR first. Run `./external/scripts/contracts/contract-check.sh`.
```
