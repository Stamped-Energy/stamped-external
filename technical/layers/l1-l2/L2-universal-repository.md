...
| Store | Waste categories served (§3.1 of [technical architecture](../../STAMPED_ARCHITECTURE.md)) | Contribution to the target |
|---|---|---|
...
| `ProductionOrder` | `order_id, line_id, due_at, priority, status, source` ([ADR-024](../../../decisions/024-026/ADR-024-holistic-plant-decisions.md)) | Deadline-aware TradeoffEngine |
...
- **No digital twin** — the graph is minimum-viable topology for attribution and routing, not an SLD/3D model (explicit non-goal per the [technical architecture](../../STAMPED_ARCHITECTURE.md) §16).
...
Append-only `ledger.mv_ledger` accepts `entry_type ∈ {realised_savings, potential_savings, opportunity_cost}` per [ledger-entry.json](../../../contracts/schemas/closure/ledger-entry.json).
