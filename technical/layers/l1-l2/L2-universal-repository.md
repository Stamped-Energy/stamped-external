...
>
> **Siblings:** [L1 — Connect & Normalise](L1-connect-and-normalise.md) · [L3 — Intelligence Core](L3-intelligence-core.md) · [L4 — Knowledge & Reasoning](L4-knowledge-and-reasoning.md) · [L5 — Closure & Verification](L5-closure-and-verification.md) · [Technical architecture](../../STAMPED_ARCHITECTURE.md)
>
> **Repo context:** `core-product/Stamped_Technical_Architecture_v1.md` Â§6 · `core-product/Stamped_Product_Definition_and_Architecture.md` Â§4.2
...
| Store | Waste categories served (Â§3.1 of [technical architecture](../../STAMPED_ARCHITECTURE.md)) | Contribution to the target |
|---|---|---|
...
| **Energy graph** | 1 (MD attribution), 2–5 (asset-level Rx) | Turns "incomer spiked" into "Furnace 3 preheat âˆ© Line 2 ramp caused it" and routes the prescription to the right named role. Attribution quality gates closure rate, and closure rate gates realised savings |
...
| **M&V & intensity ledger** | Output side of all six | The append-only record of potential vs realised ₹ + kWh + tCOâ‚‚e. This is the artefact the CFO renews on and the sustainability lead audits with. It must survive a hostile audit [19][20][31] |
...
| `ProductionOrder` | `order_id, line_id, due_at, priority, status, source` ([ADR-030](../../../decisions/028-032/ADR-030-five-domain-decision-loop.md)) | Deadline-aware TradeoffEngine |
| `PlantDepartmentGraph` | department â†’ line â†’ asset + incentive weights | Cross-shop ranking / owners |
...
|---|---|---|---|
| Plants | 1–3 | 30–50 | ICP: â‰¥ ₹30L/month bills |
| Tags per plant | 50–300 (Path B heavy) | 200–2,000 (Path A) | Series cardinality = plants Ã— tags ≤ **100k** — trivial for every candidate engine |
| Ingest rate | < 100 rows/s | ~1–5k rows/s sustained; bursts to ~20k on backfill | Even the pathological case (50 plants Ã— 2,000 tags Ã— 1 Hz = 100k rows/s) is single-node territory for TimescaleDB/ClickHouse [8][9] |
| Raw retention | 13 months | 13 months raw, 36 months aggregates | See retention table Â§4.8 |
...
- **Tenancy:** hard `org_id â†’ plant_id` isolation; no query path can cross tenants (Â§14 of the technical architecture).
...
| **"Just Postgres"** (declarative partitions, no extension) | Fine to ~10â¸ rows with manual partition + rollup jobs | Hand-rolled cron/pg_cron rollups | Native | Native | Low, but you re-implement Timescale's job system badly | Free |
...
3. **Recent-version columnstore closes the analytics gap enough**: vectorised `time_bucket()` aggregation, ColumnarIndexScan answering COUNT/MIN/MAX/FIRST/LAST from chunk metadata (up to 70–289Ã— on summary queries), bloom-filtered multi-column pruning [1][3].
...
**TimescaleDB / Tiger Data — what changed recently and why it matters here.** The 2.25–2.28 release train (Mar 2025 â†’ Jun 2026) materially changed the analytics story: ColumnarIndexScan resolves `COUNT/MIN/MAX/FIRST/LAST` from chunk-level sparse-index metadata instead of decompressing batches (up to 289Ã— on tested summary queries) [3]; `time_bucket()` grouping stays in the vectorised path end-to-end (~3.5Ã—) [3]; 2.28 derives `first(value, time)`/`last(value, time)` straight from batch metadata — exactly the "latest reading per meter" shape Stamped's dashboard hammers [2]; and continuous aggregates gained `ADD COLUMN` without rebuild, incremental batched manual refreshes, and lighter invalidation locking [2]. Licensing nuance worth stating once: the *core* is Apache 2 but compression, caggs and several policies sit under the Timescale License (source-available, free to self-host and run commercially; forbidden only to offer TimescaleDB itself as a hosted DBaaS). Stamped sells energy intelligence, not database hosting — no conflict `[~]`.

**InfluxDB 3.x — why it is disqualified despite good engineering.** The v3 engine (Rust, Arrow, Parquet-on-object-storage) is genuinely modern, but the product split works against Stamped: Core (OSS) is intentionally constrained for historical queries — and 13-month forensic lookback is a *core* Stamped requirement; Enterprise fixes that behind a per-CPU commercial license where, on expiry, **queries return errors and a stopped server will not restart** [6]. Putting a license-expiry kill-switch under the store that backs an M&V audit trail is an unacceptable continuity risk regardless of price. Add the ecosystem churn (v1 â†’ v2/Flux â†’ v3/SQL, Flux deprecated) and the migration tax lands on us twice `[!]` [5][6][7].
...
| **Total L2 data platform** | | **â‰ˆ US$1.5–2.2k/month â‰ˆ ₹1.3–1.9L/month** |

Context: that is under 1% of the *monthly electricity spend of a single ICP plant* (â‰¥ ₹30L). Infrastructure cost is not the constraint; engineering attention is — which is the strongest argument for one engine.
...
| **In-memory graph loaded from relational tables** | **Correct hot path.** L3 attribution needs "co-active equipment on shared bus within Â±3 min" many times per hour. Load the per-plant graph (a few KB) into a NetworkX/rustworkx structure in the attribution service, refresh on topology change events. Traversal cost becomes nanoseconds; Postgres stays the single writable truth |
...
| **TOD windows** | State-specific; e.g. Rajasthan FY25-26: off-peak 12:00–16:00 â†’ 10% rebate on energy charges; peak â†’ surcharge; AP extends TOD to industrial â‰¥ 15 kW with seasonal peak windows (06–10, 18–22) [25][26] | TOD windows are **date-ranged, season-ranged, category-scoped** rate rules — a versioned structure, not columns |
| **PF slabs** | Maintain avg PF â‰¥ 0.90; surcharge ~1% of energy charges per 0.01 below; incentives ~0.5% per 0.01 above 0.95/0.97 thresholds; disconnection procedure below 0.70 [25] | Piecewise-linear slab table per tariff version |
...
- *MD prescription:* recorded MD is 1,180 kVA (over CMD — attracting excess-demand surcharge in most states). A stagger prescription cuts peak to 1,060 kVA â†’ saves ₹270 Ã— 120 = **₹32,400/month** on the MD line plus the excess-demand penalty. A second-order Rx then asks: the MD histogram now peaks at 1,060 against CMD 1,100 — CMD is roughly right. But had the plant's real peak been 700 kVA, *no MD reduction below 825 kVA saves anything* until CMD itself is renegotiated — the billing-demand floor makes "reduce CMD" its own prescription category. An impact calculator without the floor overstates savings and destroys trust on the first bill reconciliation.
- *PF prescription:* average PF 0.86 on ₹40L/month energy charges â†’ surcharge = 4 steps Ã— 1% = **₹1.6L/month**, recoverable by capacitor-bank correction — often the single fastest payback in the six categories.
- *TOD prescription:* shifting 40,000 kWh/month of compressor-driven load into a 12:00–16:00 off-peak window with a 10% energy-charge rebate saves 40,000 Ã— ₹7.30 Ã— 10% â‰ˆ **₹29,200/month** — computable only if the TOD windows for *this tariff version, this season* are in the store.
...
Emission factors: CEA COâ‚‚ Baseline Database is the authoritative Indian grid factor, updated annually — v20 (Dec 2024, FY23-24: 0.727 tCOâ‚‚/MWh weighted average) and v21 (late 2025, FY24-25: ~0.710–0.7117 tCOâ‚‚/MWh provisional, all-India weighted average incl. RES & captive) [27][28]. Consequences: factors are **versioned rows** (`source=CEA, version=21.0, fy=2024-25, value, published_date`); recalculation never rewrites history — new factors apply forward; every tCOâ‚‚e figure discloses factor + version + vintage (Â§11.4 of the technical architecture). Customer overrides stored alongside with documented source.
...
4. **Tamper evidence (optional, cheap):** hash-chain columns (`seq`, `prev_entry_hash`, `entry_hash = H(prev_hash â€– canonical_row)`), appends serialised per-org via `pg_advisory_xact_lock` to prevent chain forks; periodic verifier job walks the chain [19]. Recommended from P1 when the ledger becomes customer-billing-relevant `[!]`.
...
**Backup/PITR.** Managed Postgres (RDS/Cloud SQL/Azure Flexible Server, or Tiger Cloud) in ap-south-1 gives WAL-based PITR out of the box; the untested restore is the real risk — hence restore drills in Â§5. Note one tension: hash-chained ledger + PITR restore to a point mid-chain is safe (chain is self-consistent at any prefix), but *selective* table restores must be forbidden by runbook.
...
```
 ...
 L1 topics â”€â”€â”€â”€â”€â”€â”€â”€â”€â–¶â”‚  POSTGRES 17 + TIMESCALEDB (ap-south-1)       â”‚
  measurements       â”‚                                               â”‚
  events             â”‚  schema telemetry   measurement, event        â”‚â”€â”€â–¶ L3 engines
  production         â”‚                     (hypertables + caggs)     â”‚
  bill_lines         â”‚  schema graph       asset, asset_edge         â”‚â”€â”€â–¶ L3 attribution
                     â”‚  schema commercial  tariff_version, tod_windowâ”‚    (in-mem mirror)
                     â”‚                     pf_slab, shift_calendar,  â”‚â”€â”€â–¶ L4 impact calc
                     â”‚                     production_record,        â”‚
                     â”‚                     bill, bill_line,          â”‚â”€â”€â–¶ L5 M&V
                     â”‚                     emission_factor           â”‚
                     â”‚  schema features    (caggs + dbt tables)      â”‚â”€â”€â–¶ L3/L4
                     â”‚  schema baselines   baseline (versioned,      â”‚â”€â”€â–¶ L3/L5
                     â”‚                     locked-on-cite)           â”‚
                     â”‚  schema ledger      mv_ledger (append-only,   â”‚â”€â”€â–¶ L5/L6 exports
                     â”‚                     hash-chained)             â”‚
...
CREATE TABLE telemetry.measurement (
  ...
  asset_id      uuid        NOT NULL,          -- FK â†’ graph.asset
...
- **Owner routing:** `owner_role` on equipment + a per-plant roleâ†’person map (kept in the workflow store at L5) closes the "Who" field of every prescription.
...
CREATE TABLE commercial.tod_window (
  ...
  pct_of_energy_charge numeric        -- e.g. +5% / âˆ’10% `[~]` per order
);
...
- `emission_factor` — versioned rows: `(source, version, fiscal_year, factor_tco2_per_mwh, scope, published_on, override_of uuid NULL)`. Seed: CEA v20 FY23-24 = 0.727; CEA v21 FY24-25 â‰ˆ 0.711 provisional [27][28]. New versions apply **forward only**.
...
| SEC (kWh/ton, kWh/part, kWh/batch), specific power, non-production ratio | dbt-style incremental SQL jobs joining `agg_15min` Ã— `production_record` Ã— `shift_calendar`, written to `features.*` tables with `computed_at` and `feature_set_version` | Job cadence (hourly/shift-end) | `--full-refresh` per plant; idempotent |
...
2. The moment L4 cites a baseline in a prescription's `mv_plan`, L5 flips it to `locked` (recording the citing prescription). A `BEFORE UPDATE` trigger rejects any change to a `locked` row except `status â†’ retired`.
3. Non-routine adjustments (production line added, operating-hours change) are **separate `baselines.nra` rows** referencing the locked baseline with method, quantification and approver — mirroring IPMVP NRA documentation discipline [30][31]. The M&V computation reads `locked baseline Â± NRAs`, never a mutated baseline.
...
Dual currency is structural: `kWh` fields are the physical truth, `₹` fields are `kWh Ã— the correct tariff line` (energy vs MD vs PF — computed by the L4 impact calculator against the tariff version in force), `tCOâ‚‚e = avoided grid kWh Ã— versioned CEA factor` with the factor row referenced, not inlined [27][28]. `bill_line_refs` makes every verified entry reconcilable to a physical bill — the trust anchor (Â§9.4 of the technical architecture).
...
| Backup | Automated snapshots + WAL PITR (RPO ≤ 5 min); monthly restore drill (Â§5.3); ledger partitions additionally exported to S3 with object lock (WORM) from P1 `[!]` |
...
|---|---|---|---|---|---|
| Raw telemetry (1s–1min) | `measurement` hypertable | 7 days | 7 d â†’ 13 months | **13 months** `[~]` (covers one full seasonal cycle + billing disputes) | Drop chunks; P2+: export to Parquet/S3 first `[!]` |
| Raw events/alarms | `event` hypertable | 30 days | â†’ 13 months | 13 months | Drop |
| 1-min / 15-min aggregates | caggs | 30 days | â†’ 36 months | **36 months** | Drop (day-level survives) |
...
| Baseline store | Postgres, versioned rows + lock trigger | `baseline` (draftâ†’activeâ†’lockedâ†’retired), `nra` | New version per re-fit | **Locked-on-cite, trigger-enforced** | L3 deviation, L5 M&V |
...
- **No digital twin** — the graph is minimum-viable topology for attribution and routing, not an SLD/3D model (explicit non-goal per the [technical architecture](../../STAMPED_ARCHITECTURE.md) Â§16).
- **No write-back to plant systems** — nothing in L2 addresses OT; read-only stance holds.
- **No second query engine in v1** — ClickHouse/DuckDB sidecars are P2/P3 options gated on measured benchmark breaches (Â§5.4), not defaults.
...
- **Tariff-model tests:** for each supported DISCOM template, a worked bill from the actual tariff order (e.g. JVVNL HT industrial with 75% billing-demand floor, TOD rebate, PF surcharge, FPPCA) must recompute to the printed total within Â±0.5% [25][26]. Example invariant test shape:
...
- **Emission-factor discipline tests:** every ledger entry's `avoided_tco2e` must equal `realised_kwh Ã— factor` for the *referenced* factor row; no entry may reference a factor published after the entry's period end (no retroactive factor application).

### 5.2 Data-quality SLIs (production, alerting to ops)

| SLI | Definition | Target `[~]` |
|---|---|---|
| Ingest lag | p95 event-time â†’ queryable-time, per plant | ≤ 60 s streaming; ≤ 15 min file-drop |
| Tag staleness | % of expected-active tags with no point in 2Ã— nominal interval | < 2% |
| Quality ratio | share of `estimated/bad` points per plant-day | < 5%; alert at 10% |
| Cagg freshness | `now() âˆ’ watermark` per continuous aggregate | ≤ 2Ã— refresh interval |
| Bill completeness | plants with parsed+validated bill within N days of bill date | 100% within 7 d |
| Baseline coverage | % of active assets with an `active`/`locked` baseline in scope | â‰¥ 90% by week 8 per plant |
...
Representative query pack run nightly against a production-scale synthetic dataset (50 plants Ã— 1,000 tags Ã— 13 months):

| Query | Budget p95 |
|---|---|
| 30-day 15-min profile, one asset | < 200 ms |
| MD spike forensic: 1-min incomer kVA Â± 30 min window | < 300 ms |
...
| Ledger rollup for dashboard (verified ₹/kWh/tCOâ‚‚e by month) | < 200 ms |

Budgets breached â†’ release blocked; this is the tripwire that tells us when the "one engine" bet needs the ClickHouse/lakehouse escape hatch — measured, not vibes.
...
| Phase | Exit criteria |
|---|---|
| P0 | First prescription's evidence pointers resolve end-to-end (tag â†’ window â†’ baseline â†’ ₹ line); pilot DISCOM worked-bill test passes Â±0.5%; RLS probe suite green; PITR restore drill #1 completed |
...
See [handoff/l2/core/stamped-l2-query-api-sketch.md](../../../handoff/l2/core/stamped-l2-query-api-sketch.md) for full API expansion in P1.

### Ledger entry types

Append-only `ledger.mv_ledger` accepts `entry_type âˆˆ {realised_savings, potential_savings, opportunity_cost}` per [ledger-entry.json](../../../contracts/schemas/closure/ledger-entry.json).
...
26. https://aperc.gov.in/admin/upload/TariffOrderforFY202526.pdf — AP DISCOMs FY25-26: TOD for industrial â‰¥15 kW, FPPCA pass-through
27. https://batchwise.ai/compare/cea-factor-vs-iea-factor/ — CEA v21.0 factors (FY24-25: 0.7117 tCOâ‚‚/MWh weighted avg)
