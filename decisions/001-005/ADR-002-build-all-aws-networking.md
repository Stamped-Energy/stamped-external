...
| **Status** | Accepted |
| **Date** | 2026-07-09 |
| **Supersedes** | Buy-vs-build rows in [L1 spec §4.1](../../technical/layers/l1-l2/L1-connect-and-normalise.md) class **(D)** and cost-upgrade rows that assumed Kepware/NeuronEX/EMQX Cloud |
...
| 3 | Cloud platform | **AWS only** (`ap-south-1` primary) for **`cloud` deployment mode** ([ADR-010](../006-010/ADR-010-deployment-profiles-and-portability.md)). No multi-cloud P0. |
...
ADR-002 remains **authoritative for `cloud` mode** (AWS cost-first, Mosquitto on EC2, RDS, Fargate). [ADR-010](../006-010/ADR-010-deployment-profiles-and-portability.md) adds **`local`** and **`local-dashboard`** modes without changing build-all connector strategy or plant networking invariants.
